...
class Network(nn.Module):
    ...
    #     u_decode = xu_decode[:,-7:]
    #     x_decode = xu_decode[:,:-7]
    #     u_decode = u_decode.reshape(u_decode.shape[0],7)
...
#loss function
def Klinear_loss(data,net,mse_loss,u_dim=1,gamma=0.99,Nstate=4,all_loss=0,n_SOC = 1):
    options = {}
    options["graphic"] = 0
    ...
    if all_loss:
        if ((n_SOC+1)%1000==0)or((n_SOC+1)==1):
            print("this is {} train SOC_DKN_franka, SOC is new".format(n_SOC+1))
...
def train(env_name,train_steps =10000,suffix="",all_loss=0,\
    ...
    logdir = "results/franka_data/"+suffix+"/"+subsuffix
    if not os.path.exists( "results/franka_data/"+suffix):
        os.makedirs( "results/franka_data/"+suffix)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--suffix",type=str,default="DKN_SOC_franka")
