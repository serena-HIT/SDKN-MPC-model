...
class data_collecter():
    def __init__(self,env_name) -> None:
        ...
        self.Nstates = 17
        self.uval = 0.12#franka short predict:2
        self.udim = 7
...
def Klinear_loss(data,net,mse_loss,u_dim=1,gamma=0.99,Nstate=4,all_loss=0,n_SOC = 1):
    ...
    if all_loss:
        if ((n_SOC+1)%1000==0)or((n_SOC+1)==1):
            print("this is {} train SOC_DKUC_franka, SOC is new".format(n_SOC+1))
            X_next = net.encode(data[1,:,u_dim:]).T
            A_SOC, B_SOC, err, _ = learnSOCmodel_withControl(X_current, X_next, data[0,:,:u_dim].T, options)        
...
if __name__ == "__main__":
    ...
    parser.add_argument("--suffix",type=str,default="DKUC_SOC_franka")#franka short predict : DKUC_SOC_franka_shortpredict
