...
def Klinear_loss(data,net,mse_loss,u_dim=1,gamma=0.99,Nstate=4,all_loss=0,detach=0):
    ...
    for i in range(steps-1):
        ...
        #print(bilinear.type)
        U_predecode, X_predecode = net.decode(bilinear)
...
def train(env_name,train_steps = 5000,suffix="",all_loss=0,\
    ...
    best_loss = 1000.0
    best_state_dict = {}
    logdir = "results/sizeNN_data/"+suffix+"/KoopmanNonlinear_"+env_name+"layer{}_edim{}_eloss{}_gamma{}_aloss{}_detach{}_bdim{}_samples{}".format(layer_depth,encode_dim,e_loss,gamma,all_loss,detach,b_dim,Ktrain_samples)
    if not os.path.exists( "results/sizeNN_data/"+suffix):
        os.makedirs( "results/sizeNN_data/"+suffix)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--suffix",type=str,default="DKN_newtry0125")
