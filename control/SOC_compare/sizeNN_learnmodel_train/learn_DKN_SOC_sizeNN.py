...
class Network(nn.Module):
    def __init__(self,encode_layers,bilinear_layers,Nkoopman,u_dim):#decodelinear_layers,
...
def Klinear_loss(data,net,mse_loss,u_dim=1,gamma=0.99,Nstate=4,all_loss=0,detach=0, n_SOC = 1):
    ...
    for i in range(steps-1):
        ...
        # Decodeloss += mse_loss(data[i,:,:u_dim],U_predecode) 
        beta *= gamma
    loss = loss/beta_sum
...
def train(env_name,train_steps = 5000,suffix="",all_loss=0,\
    ...
    print("layers:",layers)
    net = Network(layers,blayers,Nkoopman,u_dim)# ,dlayers
    ...
    best_loss = 1000.0
    best_state_dict = {}
    logdir = "results/SOC_compare_sizeNNdata/"+suffix+"/KoopmanNonlinear_"+env_name+"layer{}_edim{}_eloss{}_gamma{}_aloss{}_detach{}_bdim{}_samples{}".format(layer_depth,encode_dim,e_loss,gamma,all_loss,detach,b_dim,Ktrain_samples)
