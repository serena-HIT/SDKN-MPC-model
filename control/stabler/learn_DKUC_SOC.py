...
class Network(nn.Module):
    def __init__(self,encode_layers,Nkoopman,u_dim):#bicode_layers, decode_layers,
...
def Klinear_loss(data,net,mse_loss,u_dim=1,gamma=0.99,Nstate=4,all_loss=0,n_SOC = 1):
    ...
    if all_loss:
        if ((n_SOC+1)%500==0)or((n_SOC+1)==1):
            print("this is {} train DKUC, SOC is new".format(n_SOC+1))
            bilinear = data[0,:,:u_dim].T #detach's problem     
...
def train(env_name,train_steps = 10000,suffix="",all_loss=0,\
    ...
    best_loss = 1000.0
    best_state_dict = {}
    logdir = "results/SOC_data/"+suffix+"/KoopmanU_"+env_name+"layer{}_edim{}_eloss{}_gamma{}_aloss{}_samples{}".format(layer_depth,encode_dim,e_loss,gamma,all_loss,Ktrain_samples)
...
if __name__ == "__main__":
    ...
    for j in env_names:
        #for i in range(5):
            ...
            parser.add_argument("--env",type=str,default=j)
            parser.add_argument("--suffix",type=str,default="SOC_0125")
