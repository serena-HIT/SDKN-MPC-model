...
class Network(nn.Module):
    def __init__(self,encode_layers,bilinear_layers,decodelinear_layers,Nkoopman,u_dim):
        ...
        DLayers = OrderedDict()
        for layer_i in range(len(decodelinear_layers)-1):
            DLayers["linear_{}".format(layer_i)] = nn.Linear(decodelinear_layers[layer_i],decodelinear_layers[layer_i+1])
            if layer_i != len(decodelinear_layers)-2:
                DLayers["relu_{}".format(layer_i)] = nn.ReLU()
        self.decodelinear_net = nn.Sequential(DLayers)          
    ...
    def decode(self,u):
        xu_decode = self.decodelinear_net(u)
        u_decode = xu_decode[:,-1]
        x_decode = xu_decode[:,:-1]
        u_decode = u_decode.reshape(u_decode.shape[0],1)
        return u_decode,x_decode
...
#loss function
def Klinear_loss(data,net,mse_loss,u_dim=1,gamma=0.99,Nstate=4,all_loss=0,detach=0, n_SOC = 1):
    options = {}
    options["graphic"] = 0
    ...
    for i in range(steps-1):
        ...
        #print(X_current[:,:Nstate].detach().shape)
        #print(bilinear.type)
        U_predecode, X_predecode = net.decode(bilinear.T)
        #print(U_predecode.shape)
        Decodeloss += mse_loss(data[i,:,:u_dim],U_predecode) #+ mse_loss(X_current[:,:Nstate],X_predecode)
        #print(Decodeloss)
        beta *= gamma
    loss = loss/beta_sum
    Augloss = Augloss/beta_sum
    Decodeloss = Decodeloss/beta_sum
    return loss+0.5*Augloss+Decodeloss
...
def train(env_name,train_steps = 5000,suffix="",all_loss=0,\
    ...
    dlayers = [b_dim]+[layer_width]*layer_depth+[in_dim+u_dim]
    Nkoopman = in_dim+encode_dim
    print("layers:",layers)
    net = Network(layers,blayers,dlayers,Nkoopman,u_dim)
    ...
    best_loss = 1000.0
    best_state_dict = {}
    logdir = "results/SOC_data/"+suffix+"/KoopmanNonlinear_"+env_name+"layer{}_edim{}_eloss{}_gamma{}_aloss{}_detach{}_bdim{}_samples{}".format(layer_depth,encode_dim,e_loss,gamma,all_loss,detach,b_dim,Ktrain_samples)
    if not os.path.exists( "results/SOC_data/"+suffix):
        os.makedirs( "results/SOC_data/"+suffix)
    ...
    for i in range(train_steps):
        ...
        Kloss = Klinear_loss(X,net,mse_loss,u_dim,gamma,Nstate,all_loss,n_SOC = i)
        Eloss = Eig_loss(net)
        ...
        if (i+1) % eval_step ==0:
            ...
            with torch.no_grad():
                Kloss = Klinear_loss(Ktest_data,net,mse_loss,u_dim,gamma,Nstate,all_loss=0,detach=detach,n_SOC = i)
                Eloss = Eig_loss(net)
                loss = Kloss
...
if __name__ == "__main__":
    env_names = ["MountainCarContinuous-v0"]#,"CartPole-v1","MountainCarContinuous-v0","Pendulum-v1"
    for i in env_names:
        parser = argparse.ArgumentParser()
        parser.add_argument("--env",type=str,default=i)
        parser.add_argument("--suffix",type=str,default="DKN_SOC_control")
