...
root_path = "results/SOC_data/"+suffix 
print(method)
...
plt.grid(True)
plt.title("MPC_DKN_SOC Regulator")
plt.legend()
...
#     observations = np.load("D:/毕业设计/中期/Python/MPC_trykoopman/control/stabler/outcomes/DKN/"+env_name+"_"+method+"_obs.npy")
#     time_history = np.arange(steps)*env.dt
...
#             print(observations[i,-1])
#             ax[p].plot(time_history, observations[i,:].reshape(-1,1), label=labels[i])
#     ax[p].set_title(title_name,x=0.5,y=-0.40,fontsize=16)
#     #ax[p].legend()
#     ax[p].grid(True)
# plt.grid(True)
