...
def learnSOCmodel(X, Y ,options):
    ...
    S = np.eye(n)
    A_ls = np.dot(Y, pinv(X))
    nA2 = norm(Y - np.dot(A_ls, X), 'fro')**2 / 2
    O, C = polar(A_ls)  # Y = K*X, so K = Y * pinv(X)
    try:
        # sometimes, due to numerics O*C can be stable, but S\O*C*S may not
        # -- theoretically, that is not possible.
        test = eigs(np.dot(O, C), 1, which='LM', maxiter=3000, ncv=n)
        index = np.argmax(np.abs(test[0]))
        eA = np.abs(test[0])[index]
        #eA = np.abs(eigvals[0])
    except:
    ...
    if eA > 1:
        ...
        # print(B.shape)
        # print(U.shape)
        e_old = norm(Y - np.dot(solve(S, np.dot(O, np.dot(C, S))), X), 'fro')**2 / 2
    else:
        e_old = norm(Y - np.dot(A_ls, X), 'fro')**2 / 2

    maxeA = max(1, eA)
    Astab = A_ls / maxeA
    _, Stemp, Otemp, Ctemp = checkdstable(0.9999 * Astab) #, options['astab']
    etemp = norm(Y - np.dot(solve(Stemp, np.dot(Otemp, np.dot(Ctemp, Stemp))), X), 'fro')**2 / 2
    if etemp < e_old:
        S = Stemp
        O = Otemp
        C = Ctemp
    ...
    while i < options['maxiter']:
        if time.time() - begintime0 > 180000:
            break
        alpha_prev = alpha
        ...
        # print(test1)
        Z = - np.linalg.lstsq(S.T, (XYtr - np.dot(XXt, Atemp.T)).T, rcond=None)[0].T
        ...
        try:
            test2 = eigs(np.dot(On, Cn), 1, which='LM', maxiter=3000, ncv=n)
            #print(len(test2))
        ...
        # print(X.shape)
        e_new = norm(Y - np.dot(solve(Sn, np.dot(On, np.dot(Cn, Sn))), X), 'fro')**2 / 2
        ...
        while e_new > e_old and inneriter <= options['lsitermax']:
            ...
            Sn = projectInvertible(Sn, options['posdef'])
            try:
                test3 = eigs(np.dot(On, Cn), 1, which='LM', maxiter=3000, ncv=n)
                #print(test3.shape)
                #print(test3)
    ...
    Anew = A + e * A_ls
    try:
        test4 = eigs(Anew, 1, which='LM', maxiter=3000, ncv=n)
        #print(test3.shape)
        #print(test3)
        ...
        # print('Max eigenvalue not converged. Project On and Cn matrices \n')
        maxE = 2  # 任何大于1的值
    while (maxE <= 1) and np.linalg.norm(Anew - A_ls, 'fro') > 0.01:  # 不稳定的操作符
    #while maxE < 1 and np.linalg.norm(AB_ls - AB_new, 'fro')**2 / 2 > 0.01:  # 不稳定的操作符
        e = e + e_step
        Anew = A + e * grad
        try:
            test5 = eigs(Anew, 1, which='LM', maxiter=3000, ncv=n)
